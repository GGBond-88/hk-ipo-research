"""Black-box tests for L5 geo enrichment CLI.

These tests invoke ``python -m hk_ipo.enrichments.geo`` via subprocess and
...
def _make_config_override(tmp_path: Path) -> str:
    ...
    # Copy only the files necessary for the geo enrichment CLI.
    ...
    shutil.copy(enrich_src / "geo.py", enrich_dst / "geo.py")
...
def _run_geo_cli(
    *args: str,
    ...
    """Invoke ``python -m hk_ipo.enrichments.geo`` via subprocess."""
    env = dict(os.environ)
    existing = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = pythonpath_prefix + (os.pathsep + existing if existing else "")
    return subprocess.run(
        [sys.executable, "-m", "hk_ipo.enrichments.geo", *args],
        capture_output=True,
        text=True,
        timeout=timeout,
...
def test_cli_preserves_prior_enrichments(tmp_path: Path) -> None:
    """BB-L5-G-001: CLI single-file path preserves prior enrichment blocks.
    ...
    raw_record: dict[str, object] = {
        ...
        "uses": [
            {
                "use_id": "u1",
                "description": "Expand our Hong Kong operations.",
                "category_raw": "expansion",
                "percentage": 100.0,
                "source_text": "For expanding Hong Kong office space.",
            },
        ],
    }
    ...
    result = _run_geo_cli(str(cat_file), pythonpath_prefix=prefix)
    assert result.returncode == 0, f"stderr: {result.stderr}"

    # Step 4: Verify that prior enrichment (industry) was preserved.
    loaded = json.loads(enr_file.read_text(encoding="utf-8"))
    assert "industry" in loaded["enrichments"], (
        "SR-002 regression: prior 'industry' enrichment was silently dropped by CLI single-file path"
    )
    assert loaded["enrichments"]["industry"]["industries"] == ["Real Estate"]
    assert loaded["enrichments"]["industry"]["by_use_id"]["u1"] == ["Real Estate"]
    assert "geo" in loaded["enrichments"], "geo enrichment must also be present"
