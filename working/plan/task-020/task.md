# Task 020: Pipeline orchestrator — extend with skip/only/force/limit/dry-run-cost/build-frontend

## Project Overview

- **Goal:** Eight-stage CLI + React dashboard for HKEX use-of-proceeds analytics.
- **Architecture:** The orchestrator (`scripts/run_pipeline.py`) runs the full pipeline L1 -> L7 (optionally + npm build). Extended from its current L1->L4 scope.
- **Tech Stack:** Python 3.10+, pytest.

## Task Objective

Extend `scripts/run_pipeline.py` to support: `--skip` (comma-separated stage list), `--only` (comma-separated), `--limit N`, `--force`, `--dry-run-cost`, `--build-frontend`, `--db`. Wire in L4, L5, L6, L7 stages. Add `--json` structured run report.

This is Task 20 of 27.
...
- [ ] **Step 1: Verify config.PROJECT_ROOT exists before writing orchestrator code**

Run: `python -c "from hk_ipo import config; print(config.PROJECT_ROOT)"`

Expected: Outputs the absolute path to the project root (e.g., `C:\Users\...\hk-ipo-research`). This attribute is defined at line 10 of `src/hk_ipo/config.py` as `Path(__file__).parent.parent.parent`. The orchestrator uses it at L7 stage (line 233 of the orchestrator source) to resolve the `frontend/public/data` directory. If it is missing, add it to `config.py` now:

```python
PROJECT_ROOT: Path = Path(__file__).parent.parent.parent
```

- [ ] **Step 2: Write cost estimation smoke test**
...
This is a smoke check only -- the main orchestrator logic is tested in the E2E suite (Task 002 / Task 026).

- [ ] **Step 3: Replace `scripts/run_pipeline.py` with the extended version (with JSONL logging)**
...
def _log_stage_result(stage: str, stats: dict[str, Any]) -> None:
    ...
    ok = stats.get("succeeded", stats.get("passed", stats.get("loaded", stats.get("ok", 0))))
...
def _print_summary(stats: dict[str, Any]) -> None:
    ...
    for stage, s in stats.items():
        if not isinstance(s, dict):
            continue
        ok = s.get("succeeded", s.get("passed", s.get("loaded", s.get("ok", 0))))
        fail = s.get("failed", 0)
        total = s.get("total", 0)
...
def main() -> None:
    ...
    if "L1" in stages_to_run:
        ...
        stats["L1"] = {"ok": n_sections, "failed": 0, "total": n_sections}
        _log_stage("L1", "ok" if n_sections > 0 else "skipped")
        l1_attempted = {p.stem for p in pdf_dir.glob("*.pdf")}
        l1_succeeded = _tickers_from_dir(sections_dir)
        # PR-012 fix: count attempted-but-not-succeeded as failures
        l1_failed_count = len(l1_attempted - l1_succeeded)
        stats["L1"]["failed"] = l1_failed_count
    ...
    for stage_key in stages_to_run:
        ...
        mod.run(categorized_dir, enriched_dir, all_files=True, force=args.force)
        n_enriched = len(list(enriched_dir.glob("*.json")))
        stats[stage_key] = {"ok": n_enriched, "failed": 0}
        _log_stage(stage_key, "ok" if n_enriched > 0 else "skipped")
    ...
    if "L6" in stages_to_run:
        ...
        stats["L6"] = l6_process_all(enriched_dir, db_path)
        _log_stage_result("L6", stats["L6"])
        l6_attempted = _tickers_from_dir(enriched_dir)
        _record_stage_sweep(db_path, run_id, "L6", l6_attempted, l6_attempted)
    ...
    if args.build_frontend:
        ...
        for cmd in (["npm", "install"], ["npm", "run", "build"]):
            ...
            if result.returncode != 0:
                ...
                print(f"  OK: {' '.join(cmd)}")
        stats["frontend"] = {"ok": 1 if result.returncode == 0 else 0}
        _log_stage("frontend", "ok" if result.returncode == 0 else "failed")
...
Run: `python -c "import scripts.run_pipeline"`

Expected: No import errors. (May produce a warning about import path, which is fine.)

- [ ] **Step 5: Run `--dry-run-cost` with a fake API key**

Run (PowerShell):
```powershell
$env:OPENROUTER_API_KEY = "broken-on-purpose"
python scripts/run_pipeline.py --dry-run-cost --pdf-dir data/raw_pdfs
```

Expected: Prints a JSON with `estimated_tokens` and `estimated_cost_usd`, exits 0. No API calls made.
